...
var perf_result = attach(perf_prog, perf_attr)
```

**Context-specific implementations:**
- **eBPF:** Not available
- **Userspace:** Uses `attach_bpf_program_by_fd` for standard targets and generated `attach_perf_by_attr` for perf events
- **Kernel Module:** Not available

See `perf/PERF_ATTR.md` for the current `perf_event_attr` field meanings and implementation boundaries.
