...
struct packet_filter_ebpf *obj = NULL;
...
int main(void) {
    uint32_t __func_call_2;
    uint32_t __func_call_3;
    uint32_t __func_call_0;
    uint32_t __func_call_1;
    // No arguments to parse
    // Implicit eBPF skeleton loading - makes global variables immediately accessible
    if (!obj) {
        obj = packet_filter_ebpf__open_and_load();
        if (!obj) {
    ...
    int32_t var_prog;
    var_prog = get_bpf_program_handle("packet_filter");
    if (var_prog < 0) {
        fprintf(stderr, "Failed to get BPF program handle\n");
        return 1;
    }
    __func_call_0 = attach_bpf_program_by_fd(var_prog, "eth0", 0);
    __func_call_1 = printf("Packet filter attached to eth0\n");
    __func_call_2 = printf("Filtering incoming packets...\n");
    detach_bpf_program_by_fd(var_prog);
    __func_call_3 = printf("Packet filter detached\n");
    return 0;
}
